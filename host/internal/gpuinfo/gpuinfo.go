...
func (r Reader) Read() Result {
	...
	if oddPower {
		out.NoPower = lang.Say("драйвер отдаёт мощность карты не в микроваттах, как обещает документация hwmon: полученное число меньше полуватта, то есть единица измерения у него другая — такое число мы не печатаем")
	}
	return out
}
