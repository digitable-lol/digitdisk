...
func init() {
	add(map[string]string{
		...
		"  --live            status: живой экран; без терминала — ошибка":              "  --live            status: the live screen; an error without a terminal",
		"  --plain           status: один снимок, даже когда вывод в терминал":         "  --plain           status: one snapshot, even into a terminal",
		...
		"  --protect ЧТО     clean: не трогать путь или «разряд:кэш»; можно повторять": "  --protect WHAT    clean: spare a path or «разряд:кэш»; may repeat",
		"  --protect-file Ф  clean: защитный список файлом":                            "  --protect-file F  clean: the protect list as a file",
