...
копилефта в платном архиве Workbench.
  ...
  5. Удаление живёт в одном каталоге и не бывает рекурсивным. Раньше здесь
     стоял запрет: в дереве не было ни одного вызова удаления, и это легко
     проверялось. Инструмент, который показывает, что убрать, и не умеет
     убрать, — половина инструмента, поэтому запрет снят и заменён правилом.
     Правило жёстче, чем кажется: вызовы, стирающие и переставляющие файлы,
     разрешены ровно в `host/internal/clean/` — там, где вокруг них стоят
     приговор ядра, отпечаток файла, корзина и журнал, — а `RemoveAll`
     запрещён всюду без исключения. Рекурсивное удаление превращает ошибку в
     одну строку в катастрофу на весь каталог; способ не иметь такой ошибки —
     не иметь такого вызова.
...
# Единственный каталог, которому позволено стирать и переставлять файлы. Всё,
# что там лежит, обязано пропускать удаление через приговор ядра «МожноУбрать»,
# сверку отпечатка, корзину и журнал — см. host/internal/clean/clean.go.
REMOVAL_HOME = "host/internal/clean/"

# Вызовы, которые уносят или затирают данные пользователя. Ищутся со скобкой,
# чтобы имя в комментарии или в строке отчёта не считалось вызовом.
REMOVAL_CALLS = ("Remove(", "Rename(", "Truncate(")

# Рекурсивное удаление. Исключений нет и не предполагается: ни в host, ни в
# напечатанном ядре, ни в испытаниях.
RECURSIVE_REMOVAL = "RemoveAll"
...
# 5. Удаление — только в host/internal/clean/, и никогда рекурсивное.
#
...
removal_hits = []
for relative in files:
    if Path(relative).suffix != ".go" or not scanned(relative):
        continue
    try:
        text = read(relative)
    except (OSError, UnicodeError):
    ...
    if RECURSIVE_REMOVAL in text:
        removal_hits.append(
            f"{relative}: содержит «{RECURSIVE_REMOVAL}» — рекурсивное удаление "
            f"запрещено во всём дереве без исключений. Ошибка в одной строке "
            f"сносит каталог целиком; digitdisk стирает файлы по одному, по "
            f"списку в журнале, и пустые каталоги — только через Remove, "
            f"который непустой каталог не трогает."
        )
        continue

    if relative.startswith(REMOVAL_HOME):
        continue
    for call in REMOVAL_CALLS:
        if call in text:
            removal_hits.append(
                f"{relative}: содержит «{call}» вне {REMOVAL_HOME} — стирать и "
                f"переставлять файлы вправе только этот каталог, где вокруг вызова "
                f"стоят приговор ядра «МожноУбрать», сверка отпечатка, корзина и "
                f"журнал. Удаление в обход них — не возможность, а дефект."
            )
            break

if removal_hits:
    failures.extend(removal_hits)
else:
    go_files = [f for f in files if Path(f).suffix == ".go" and scanned(f)]
    passed.append(
        f"удаление только в {REMOVAL_HOME}, рекурсивного нет ни в одном "
        f"из {len(go_files)} Go-исходников"
    )
